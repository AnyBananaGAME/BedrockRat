...
const { toNotchianYaw, toNotchianPitch, fromNotchianPitch, fromNotchianYaw } = require("../../../utils/conversions");
...
module.exports = (client) => {
  client.express = () => {
    ...
    app.post("/api/getEntities", async (req, res) => {
      ...
      const entities = [];
      const list = client.findEntities(radius).map(entity => {
    ...
    wss.on("connection", (ws) => {
      ...
      client.on("pre_player_auth_input", (event) => {
        if(ws) {
          ...
          ws.send(`${JSON.stringify({type: "position", position: {x: x, y: y ,z: z}})}`)
          ws.send(`${JSON.stringify({type: "head", yaw: yaw, pitch: pitch})}`)  
        }
      })
        ...
        const data = JSON.parse(message);
          ...
          switch (data.type) {
            case "getBlock":
              ...
              const block = await client.world.getBlock(pos);
              ws.send(JSON.stringify({type: "blockResponse", block: block, position: data.position}))
              break;
